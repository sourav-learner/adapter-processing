...
public class EVDMonthlyFileExecutor extends EVDMonthlyFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(EVDMonthlyFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;

    private boolean applyDuplicateCdrCheck = false;
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = {"dealer_id","trans_time","balance_after","reference_no"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', true, true)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateEVDMonthlyCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                handleEvents("EVD_MONTHLY_FCT", data);
            }else {
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[EVD_MONTHLY_FCT] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addEVDMonthlyRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeEVDMonthlyCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
