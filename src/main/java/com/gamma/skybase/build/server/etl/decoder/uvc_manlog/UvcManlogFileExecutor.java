...
public class UvcManlogFileExecutor extends UvcManlogFileProcessor {

    private static final Logger logger = LoggerFactory.getLogger(UvcManlogFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;
    String[] headers = new String[]{"SN", "STREAMNUMBER", "BATCHNO", "SEQUENCE", "FACEVALUE", "ERRORTYPE", "CURRENCY", "CARDSTARTDATE", "CARDSTOPDATE", "CURDISTRIBUTOR", "RECHARGENUMBER", "CALLINGNUMBER", "TRADETIME", "VCNO", "TRADETYPE", "RESID1", "RESCHARGE1", "RESACTIVEDAYS1", "RESID2", "RESCHARGE2", "RESACTIVEDAYS2", "CARDCOSID", "TENANTID", "SRV_TYPE_KEY"};
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_UVC_MANLOG", data);
            }
        }
    }
}
