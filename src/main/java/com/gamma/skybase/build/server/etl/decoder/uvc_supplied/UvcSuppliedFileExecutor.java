...
public class UvcSuppliedFileExecutor extends UvcSuppliedFileProcessor {

    private static final Logger logger = LoggerFactory.getLogger(UvcSuppliedFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;
    String[] headers = new String[]{"ACCOUNTNUMBER", "PINNUMBER", "BATCHNO", "SEQUENCE", "FACEVALUE", "CURRENCY", "CARDSTARTDATE", "CARDSTOPDATE", "HOTCARDFLAG", "USESTATE", "CURDISTRIBUTOR", "RESNUM", "RESID1", "RESLEFT1", "RESID2", "RESLEFT2", "RESID3", "RESLEFT3", "RESID4", "RESLEFT4", "RESID5", "RESLEFT5", "OPRDATETIME", "OPERATOR", "TRADETIME", "CHARGENUMBER", "OPRTYPE", "CARDSTOPDATEBAK", "AUTHAVALIBLETIME", "CARDCOSID", "CARDSPID", "PACKAGEID", "CARDTENANTID", "RECHGNUMTENANTID", "TIMEZONE", "CALLINGNUMBER", "PARTNUMBER", "CREATEDATE", "CBPSESSIONID", "CHANNELTYPE"};
...
//                    record.put("RECHARGE_TYPE", metadata.decompFileName);
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_UVC_SUPPLIED", data);
            }
        }
    }
}
