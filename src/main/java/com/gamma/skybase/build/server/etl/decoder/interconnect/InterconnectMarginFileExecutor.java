...
public class InterconnectMarginFileExecutor extends InterconnectMarginFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(InterconnectMarginFileExecutor.class);
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = new String[]{"event_time", "served_msisdn", "other_msisdn", "rated_flat_amount", "duration_volume", "entry_date_timestamp", "trunk_group_in_address", "trunk_group_out_address"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', headers, 1)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateInterconnectMarginCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                handleEvents("INTERCONNECT_MARGIN_FCT", data);
            }else {
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[INTERCONNECT_MARGIN_FCT] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addInterconnectMarginRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeInterconnectMarginCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
