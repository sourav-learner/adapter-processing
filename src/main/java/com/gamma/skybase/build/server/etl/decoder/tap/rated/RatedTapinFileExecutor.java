...
public class RatedTapinFileExecutor extends RatedTapinFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(RatedTapinFileExecutor.class);
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = {"event_date","start_time_timestamp","start_time_offset","s_p_number_address","s_p_equipment_number","s_p_port_address",
                "o_p_number_address","duration_volume","rated_volume","rounded_volume","call_type","rated_flat_amount","tariff_info_sncode","xfile_ind",
                "imp_party1_bill_cycle","net_element_address","bop_tariff_info_sncode"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', true, true)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateRatedTapinCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                handleEvents("RATED_TAPIN_FCT", data);
            }else {
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[Rated-TAPIN] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addRatedTapinRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeRatedTapinCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
