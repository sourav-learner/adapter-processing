...
public class EVDFileExecutor extends EVDFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(EVDFileExecutor.class);
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = {"dealer_id","operation_type","operation_desc","trans_time","amount","balance_after","reference_no","request_for","serial","failure_code","flex_fld1","flex_fld2","valid_balance","valid_dt","invalid_balance","invalid_dt","flex_fld3","no_response","src_transid","air_transid","dest_balance_before","dest_balance_after","source_balance_before"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', true, true)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateEVDCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[EVD_FCT] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addEVDRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeEVDCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
