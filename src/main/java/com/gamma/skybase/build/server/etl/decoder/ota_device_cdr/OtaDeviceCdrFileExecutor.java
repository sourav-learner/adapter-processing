...
public class OtaDeviceCdrFileExecutor extends OtaDeviceCdrFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(OtaDeviceCdrFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;
    String[] headers = new String[]{"RECTYPE" , "IMSI" , "TIMESTAMP" , "IMEI" , "MSISDN" , "OPERATIONTYPE" , "PREVIOUSVALUE" , "PROCS_FILE" , "PROCS_TIME"};
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_OTA_DEVICE_CDR", data);
            }
        }
    }
