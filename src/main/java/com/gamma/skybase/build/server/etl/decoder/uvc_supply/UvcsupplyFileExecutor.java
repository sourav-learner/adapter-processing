...
public class UvcsupplyFileExecutor extends UvcsupplyFileProcessor {

    private static final Logger logger = LoggerFactory.getLogger(UvcsupplyFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;
    String[] headers = new String[]{"ACCOUNTNUMBER", "BATCHNO", "SEQUENCE", "FACEVALUE", "CURRENCY", "CARDSTARTDATE", "CARDSTOPDATE", "HOTCARDFLAG", "USESTATE", "CURDISTRIBUTOR", "TRADETIME", "OPRTYPE", "CARDCOSID", "CARDSPID", "CARDTENANTID", "CHARGENUMBER", "SRV_TYPE_KEY"};
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_UVC_SUPPLY", data);
            }
        }
    }
}
