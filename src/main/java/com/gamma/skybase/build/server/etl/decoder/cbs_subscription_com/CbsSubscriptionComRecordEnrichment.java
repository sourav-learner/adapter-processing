...
public class CbsSubscriptionComRecordEnrichment implements IEnrichment {
    ...
    public MEnrichmentResponse transform(MEnrichmentReq request) {
        ...
        CbsSubscriptionComEnrichmentUtil tx = CbsSubscriptionComEnrichmentUtil.of(record);

//        CDRTYPE
        Optional<String> cdrType = tx.getCdrType();
        cdrType.ifPresent(s -> record.put("CDRTYPE", s));
...
//        EVT_SOURCE_CATEGORY
        Optional<String> evtSourceCategory = tx.getEvtSourceCategory();
        evtSourceCategory.ifPresent(s -> record.put("EVTSOURCE_CATEGORY", s));
...
//        CHARGING_USAGE_TYPE
        Optional<String> chargingUsageType = tx.getChargingUsageType();
        chargingUsageType.ifPresent(s -> record.put("CHARGING_USAGE_TYPE", s));

//        SERVICE_CAPABILITY
        Optional<String> serviceCapability = tx.getServiceCapability();
        serviceCapability.ifPresent(s -> record.put("SERVICE_CAPABILITY", s));

//        PROVISION_TYPE
        Optional<String> provisionType = tx.getProvisionType();
        provisionType.ifPresent(s -> record.put("PROVISION_TYPE", s));

//        UPLINK_VOLUME
        Optional<Long> uplinkVolume = tx.getUplinkVolume("InputOctets");
        uplinkVolume.ifPresent(s -> record.put("UPLINK_VOLUME", s));

//        DOWNLINK_VOLUME
        Optional<Long> downlinkVolume = tx.getDownLinkVolume("OutputOctets");
        downlinkVolume.ifPresent(s -> record.put("DOWNLINK_VOLUME", s));

//        TOTAL_VOLUME
        Optional<Long> totalVolume = tx.getTotalVolume("TotalOctets");
        totalVolume.ifPresent(s -> record.put("TOTAL_VOLUME", s));

//        CALLING_NETWORK_TYPE
        Optional<String> callingNetworkType = tx.getCallingNetworkType();
        callingNetworkType.ifPresent(s -> record.put("CALLING_NETWORK_TYPE", s));
...
//        START_TIME_OFBiILL_CYCLE
        Optional<String> startTimeOfBillCycle = tx.getStartTimeOfBillCycle("StartTimeOfBillCycle");
        startTimeOfBillCycle.ifPresent(s -> record.put("START_TIME_OFBiILL_CYCLE", s));

//        REQUEST_ACTION
        Optional<String> requestAction = tx.getRequestAction();
        requestAction.ifPresent(s -> record.put("REQUEST_ACTION", s));
