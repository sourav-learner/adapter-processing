...
public class EVDSaleFileExecutor extends EVDSaleFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(EVDSaleFileExecutor.class);
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = {"paymentid","billingcycle","paymentdate","paymenttype","amount","description","dealerindex","chequedate","bank","paymentlevel","username","depositedbank","transactiontime","paymentorder","reference","terminal"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', true, true)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateEVDSaleCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                handleEvents("EVD_SALE_FCT", data);
            }else {
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[EVD_SALE_FCT] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addEVDSaleRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeEVDSaleCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
