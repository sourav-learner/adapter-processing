...
public class PcrfSubQuotaFileExecutor extends PcrfSubQuotaFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(PcrfSubQuotaFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;
    String[] headers = new String[]{"SID" , "SUBSCRIBERSN" , "QUOTANAME" , "INITIALVALUE" , "BALANCE" , "CONSUMPTION" , "STATUS" , "NEXTRESETDATETIME" , "LASTRESETDATETIME"};
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_PCRF_SUBQUOTA", data);
            }
        }
    }
