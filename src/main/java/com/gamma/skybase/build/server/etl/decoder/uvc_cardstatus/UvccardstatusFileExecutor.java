...
public class UvccardstatusFileExecutor extends UvccardstatusFileProcessor{

    private static final Logger logger = LoggerFactory.getLogger(UvccardstatusFileExecutor.class);
    private List<LinkedHashMap<String, Object>> jsonRecords;

    String[] headers = new String[]{"FACEVALUE", "HOTCARDFLAG", "CARDTOTAL", "BATCHNO", "TENANTID", "CARDCOSID", "UNKNOWN"};
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                if (data != null)
                    handleEvents("FCT_UVC_CARDSTATUS", data);
            }
        }
    }
}