...
public class MSCRecordEnrichment implements IEnrichment {
    ...
    private static final String FORMAT4 = "yyyyMMdd";
    ...
    private static final ThreadLocal<SimpleDateFormat> yyyyMMdd = ThreadLocal.withInitial(() -> new SimpleDateFormat(FORMAT4));

    private final Date currentTime = new Date();
    private static final Logger logger = LoggerFactory.getLogger(MSCRecordEnrichment.class);
    private final Pattern regularTextPattern = Pattern.compile("[a-zA-Z0-9]*");
...
//
            ...
            try {
                ...
                if (eventStartTime != null) {
                    record.put("EVENT_START_TIME", sdfT2.get().format(eventStartTime)); // 2
                    record.put("EVENT_START_TIME_KRT", sdfT.get().format(eventStartTime)); // 2
            ...
            if (eventEndTime != null) record.put("EVENT_END_TIME", sdfT2.get().format(eventEndTime));
            ...
            if (duration == 0) record.put("ZERO_DURATION_IND", "1");
            else record.put("ZERO_DURATION_IND", "0");
            ...
            if (!otherMSISDN.equals("")) {
                if(otherMSISDN.startsWith("e") || otherMSISDN.startsWith("E")){
                    try {
                        String decodedSpecialNum = transformationLib.decodeSpecialNumbers(otherMSISDN);
                        if(isPureAscii(decodedSpecialNum)){
                            otherMSISDN = decodedSpecialNum;
                        }
                    } catch(Exception e){
                        logger.info(otherMSISDN + " can not be converted to special number");
                    }
                    record.put("ORIGINAL_A_NUM", otherMSISDN);
                }else {
                    otherMSISDN = transformationLib.normalizeMSC(otherMSISDN, countryCode);
                }
                record.put("OTHER_MSISDN", otherMSISDN);
            }
            if (StringUtils.isNotEmpty(servedMSISDN)) {
                servedMSISDN = transformationLib.normalizeMSC(servedMSISDN, countryCode);
            }else {
                servedMSISDN = "UNKNOWN";
            }
            ...
            if (incomingRoute != null) record.put("IN_TG_ID_KEY", incomingRoute.toString());

            Object outgoingRoute = record.get("outgoingRoute");
            if (outgoingRoute != null) record.put("OUT_TG_ID_KEY", outgoingRoute);
...
//            if ("3".equalsIgnoreCase(srvTypeKey)){
//                logger.info("inroamer cdr.");
//            }
            ...
            if (servedMSISDN != null && !servedMSISDN.equals("")) {
                ...
                if (ddk != null) {
                    ...
                    record.put("SERVED_OPER", ddk.getProviderDesc());
                }
            }
    ...
    private boolean isPureAscii(String text) {
        return regularTextPattern.matcher(text).matches();
    }
}
