package com.gamma.skybase.build.server.etl.decoder.interconnect;
...
public class InterconnectFileExecutor extends InterconnectFileProcessor {
    private static final Logger logger = LoggerFactory.getLogger(InterconnectFileExecutor.class);
    ...
    public void parseFile(String fileName) throws Exception {
        ...
        String [] headers = {"event_date","cust_info_customer_id","trunk_group_in_address","trunk_group_out_address",
                "rated_flat_amount_currency","call_type","tariff_info_usage_ind","tariff_info_sncode","calls_count",
                "rated_flat_amount","duration_volume", "entry_date_timestamp"};
        ...
        try (DelimitedFileDecoder decoder = new DelimitedFileDecoder(fileName, ',', headers, 1)) {
            ...
            while (decoder.hasNext()) {
                try {
                    ...
                    String cdrKey = SkybaseCacheServiceHandler.instance().generateInterconnectCdrKey(record, dataSource.getAdapterName().toLowerCase());
                    recCount++;
    ...
    private void processRecord(LinkedHashMap<String, Object> record, IEnrichment enrichment) throws Exception {
        if (enrichment != null) {
            ...
            if (response.isResponseCode()) {
                ...
                handleEvents("INTERCONNECT_FCT", data);
            }else {
    ...
    private boolean isDuplicateRecord(String cdrKey) {
        if (applyDuplicateCdrCheck) {
            if (SkybaseCacheServiceHandler.instance().checkIfDuplicateCDR(cdrKey)) {
                if (logger.isDebugEnabled()) logger.debug("[INTERCONNECT_FCT] Duplicate cdr found");
                return true;
            }
        }
        return false;
    ...
    public void addCdrKey(String cdrKey, Map<String, Object> data) {
        if (applyDuplicateCdrCheck) {
            SkybaseCacheServiceHandler.instance().addInterconnectRecordsInCdrContainer(dataSource.getAdapterName().toLowerCase(), Arrays.asList(data));
        }
    }
    ...
    public void flush() throws SkybaseDeserializerException {
        ...
        /* cache flush */
        SkybaseCacheServiceHandler.instance().writeInterconnectCDREntries(dataSource.getAdapterName().toLowerCase(), null, true);
